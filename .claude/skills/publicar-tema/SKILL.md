...
- **Estratégia de conteúdo:** `projetos/<PROJETO>/marketing/seo/05-estrategia-conteudo.md` (lista mestra de temas, criada pelo `/seo`)
- **Outras pesquisas SEO:** `projetos/<PROJETO>/marketing/seo/01-pesquisa-demanda.md`, `02-analise-concorrencia.md`, `08-geo-otimizacao-ia.md`
- **Skill carrossel:** `.claude/skills/carrossel/SKILL.md` — usar pra fase do carrossel
- **Site (blog):** `projetos/<PROJETO>/site/` — destino dos artigos. Estrutura comum: Astro em `site/astro-site/src/content/blog/`, ou WordPress, ou outro. Se ainda não tiver site, perguntar antes
- **Tom de voz:** `projetos/<PROJETO>/_memoria/preferencias.md`
- **Contexto:** `projetos/<PROJETO>/_memoria/empresa.md`, `projetos/<PROJETO>/identidade/design-guide.md`

---

## Workflow

### Passo 0 — Identificar projeto e tema

**Identificar projeto:** Se o usuário mencionou um projeto/cliente explicitamente, identificar o slug da pasta em `projetos/`. Se não for claro, listar as pastas em `projetos/` e perguntar: "Qual projeto?". Definir `BASE = projetos/<projeto>`.

**Escolher tema:** Se o usuário passou um tema explícito → usar.

Se não passou nada → ler `{BASE}/marketing/seo/05-estrategia-conteudo.md`, listar os artigos satélite + a página pilar, e perguntar:
...
- Astro: `{BASE}/site/astro-site/src/content/blog/<slug>.md`
...
author: "<nome configurado em {BASE}/_memoria/empresa.md>"
keywords:
  - keyword principal
  - variação 1
  - variação 2
...
**Regras de escrita** (seguir `{BASE}/_memoria/preferencias.md` estritamente):
...
**Pasta:** `{BASE}/marketing/conteudo/<slug-do-blog>-<YYYY-MM-DD>/`
...
**Capa:** seguir sequência alternada do feed (claro → foto/escuro → cor principal → repete) — checar `{BASE}/marketing/conteudo/` mais recente.

### Passo 4 — Legendas (3 versões)

Salvar todas em `{BASE}/marketing/conteudo/<pasta-do-carrossel>/`:
...
✓ Carrossel: {BASE}/marketing/conteudo/<pasta>/
...
3. Renderizar PNGs do carrossel: cd {BASE}/marketing/conteudo/<pasta> && node render.js
