...
- **Projeto ativo:** `projetos/<PROJETO>/` — identificado no Passo 0
- **Identidade visual:** `projetos/<PROJETO>/identidade/design-guide.md` — LER ANTES de criar qualquer visual
- **Contexto do negócio:** `projetos/<PROJETO>/_memoria/empresa.md`
- **Tom de voz:** `projetos/<PROJETO>/_memoria/preferencias.md`
...
- **Outputs vão em:** `projetos/<PROJETO>/marketing/conteudo/<tipo>-<tema>-<YYYY-MM-DD>/`
...
Antes de criar HTML: ler `{BASE}/identidade/design-guide.md`. Se estiver em branco, usar o "Estilo visual base" acima como default.
...
Seguir `{BASE}/_memoria/preferencias.md`. Em geral: frases naturais, sem jargão de marketing, sem corporativês. O público real raramente fala "ticket médio", "performance", "B2B". Falar como ele fala.
...
Antes de qualquer leitura, determinar qual projeto está sendo trabalhado:

1. Se o usuário mencionou um projeto/cliente explicitamente no pedido (ex: "carrossel pra Sargentão", "post do HAOA"), identificar o slug da pasta correspondente em `projetos/`
2. Se não for claro, listar as pastas em `projetos/` e perguntar: "Qual projeto? (lista)"
3. Definir `BASE = projetos/<projeto>` — usar esse prefixo em **todos** os caminhos abaixo

### Passo 1 — Entender e planejar

1. Ler `{BASE}/_memoria/preferencias.md` e `{BASE}/_memoria/empresa.md`
2. Ler `{BASE}/identidade/design-guide.md` pra cores, fontes e logo
...
```bash
node --env-file=.env scripts/gerar-imagem.js "PROMPT" "{BASE}/marketing/conteudo/<pasta>/foto-<nome>.png"
```
...
1. **Formato mestre primeiro (sequencial, com cuidado).** Pra cada slide do mestre, criar um arquivo `{BASE}/marketing/conteudo/<pasta>/<formato>/slide-NN.html` (`slide-01.html`, `slide-02.html`, ...). Cada HTML é **autocontido**: inline CSS, Google Fonts via `<link>` no head (única dependência externa), referência a fotos por path relativo (`../foto-x.png` ou `foto-x.png` se a foto estiver dentro da pasta do formato). Aplicar:
   ...
   - Cores e tipografia de `{BASE}/identidade/design-guide.md`
...
```
{BASE}/marketing/conteudo/<tipo>-<tema>-<YYYY-MM-DD>/
...
- Sempre ler `{BASE}/identidade/design-guide.md` antes de criar qualquer visual
...
- Linguagem segue `{BASE}/_memoria/preferencias.md` estritamente
