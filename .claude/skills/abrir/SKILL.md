...
0. **Identificar projeto:**
   - Se o usuário passou um nome de projeto (ex: `/abrir haoa`, "abre o Sargentão"), identificar o slug em `projetos/`
   - Se não passou, listar as pastas em `projetos/` e perguntar: "Qual projeto?" — só perguntar se houver mais de um. Se houver apenas um, abrir automaticamente
   - Definir `BASE = projetos/<projeto>`

1. Ler, em ordem:
   - `{BASE}/_memoria/empresa.md`
   - `{BASE}/_memoria/preferencias.md`
   - `{BASE}/_memoria/estrategia.md`
   - `{BASE}/identidade/design-guide.md` (só pra saber se está preenchido ou em branco)

2. Se algum dos três primeiros estiver em branco (placeholder), responder:
   > "Vi que `{BASE}/_memoria/<arquivo>.md` ainda não foi preenchido. Quer rodar `/instalar` agora?"
   E parar.
