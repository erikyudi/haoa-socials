...
export async function editSlide(item, slideIdx, pedido) {
  ...
  const timer = setInterval(() => {
    paintSlideStatus(slideIdx, 'gerando · ' + Math.floor((Date.now() - startedAt) / 1000) + 's', 'running');
  }, 500);
  state.slideRuns[key] = { runId, startedAt, timer };

  paintSlideBusy(slideIdx, true);
  paintSlideStatus(slideIdx, 'gerando imagem…', 'running');

  const brandName = state.business?.name || 'da marca';
  const isHtml = /\.html?$/i.test(slidePath);
  const prompt = isHtml
    ? `TAREFA TÉCNICA AUTOMÁTICA — NÃO leia arquivos de memória (_memoria/), NÃO verifique skills antes de agir. Execute diretamente.

Edite UM ÚNICO slide HTML de um carrossel do ${brandName}.

Post: ${item.name}
Pasta do post: ${getActiveFolder(item) || ''}
Arquivo HTML a editar: ${slidePath}

Pedido do usuário: "${pedido}"

REGRAS RÍGIDAS — LEIA ANTES DE AGIR:
1. O ÚNICO arquivo que você pode modificar é: ${slidePath}
2. NÃO toque em NENHUM outro arquivo da mesma pasta. Os outros slides do carrossel devem ficar EXATAMENTE como estão.
...
4. Use as ferramentas Read e Edit/Write diretamente no arquivo HTML. NÃO escreva scripts externos.
5. Pode LER outros slides pra entender o estilo, mas NÃO ESCREVER neles.
6. Mantenha a estrutura HTML existente — modifique apenas o necessário para atender o pedido.
7. IMAGENS LOCAIS: para referenciar imagens locais no HTML use o formato /api/file?path=<caminho>, ex: <img src="/api/file?path=uploads/foto.jpg">
8. Responda com UMA frase curta confirmando o que foi feito nesse slide.`
    : `TAREFA TÉCNICA AUTOMÁTICA — NÃO leia arquivos de memória (_memoria/), NÃO verifique skills antes de agir. Execute diretamente.

Edite UM ÚNICO slide específico de um carrossel do ${brandName}.
...
7. Responda com UMA frase curta confirmando o que foi feito nesse slide.`;
  ...
  if (visibleSameItem) {
    ...
    if (ok) {
      ...
      const ts = Date.now();
      const baseUrl = fileUrl(slidePath);
      const bustedUrl = baseUrl + (baseUrl.includes('?') ? '&' : '?') + 't=' + ts;

      // Atualiza img ou iframe no lightbox (depende do tipo de slide)
      const slideCell = document.getElementById('slide-cell-' + slideIdx);
      const imgEl = slideCell ? slideCell.querySelector('.slide-img') : null;
      if (imgEl) imgEl.src = bustedUrl;
      const frameEl = slideCell ? slideCell.querySelector('.slide-frame') : null;
      if (frameEl) frameEl.src = bustedUrl;

      // Atualiza thumbnail na biblioteca se o slide editado é o capa (índice 0)
      const activeSlides = getActiveSlides(item);
      const isCoverSlide = slideIdx === 0 ||
        (activeSlides[slideIdx] && activeSlides[slideIdx] === (item.slides && item.slides[0]));
      if (isCoverSlide) {
        const libIdx = state.library.findIndex(x => x.name === item.name);
        if (libIdx >= 0) {
          const card = document.querySelector(`.lib-card[data-lib="${libIdx}"]`);
          const cover = card ? card.querySelector('.cover') : null;
          if (cover) cover.style.backgroundImage = `url('${bustedUrl}')`;
        }
      }
    } else {
