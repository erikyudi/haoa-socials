...
import { MODELS, setSlideModel } from '../core/persist.js';
...
function _restoreSlideRuns(item) {
  ...
  _getActiveSlides(item).forEach((_, i) => {
    ...
    paintSlideStatus(i, 'gerando · ' + Math.floor((Date.now() - run.startedAt) / 1000) + 's', 'running');
  });
}
...
export async function openLightbox(itemIdx, slideIdx = 0) {
  ...
  if (sideEl) {
        ...
        <div class="ig-edit-forms">
          ${slides.map((_, i) => `
            <form class="ig-edit-form${i === 0 ? ' active' : ''}" data-slide="${i}" id="slide-edit-form-${i}">
              ...
              <input type="file" id="slide-file-${i}" accept="image/*" style="display:none">
              <button type="button" id="slide-file-btn-${i}" style="margin-top:5px;width:100%;background:transparent;border:1px solid rgba(245,240,232,0.18);color:rgba(245,240,232,0.7);padding:6px 10px;border-radius:6px;font:12px var(--sans);cursor:pointer;text-align:left;" title="Inserir imagem neste slide">📎 Inserir imagem</button>
    ...
    slides.forEach((_, i) => {
      ...
      const fileBtn = document.getElementById(`slide-file-btn-${i}`);
      const fileInput = document.getElementById(`slide-file-${i}`);
      if (fileBtn && fileInput) {
        fileBtn.addEventListener('click', () => fileInput.click());
        fileInput.addEventListener('change', async (e) => {
          const file = e.target.files[0];
          if (!file) return;
          const statusEl = document.getElementById('slide-status-' + i);
          if (statusEl) { statusEl.textContent = 'Enviando imagem…'; statusEl.className = 'lightbox-slide-status'; }
          fileBtn.disabled = true;
          try {
            const imgPath = await _uploadSlideImage(file);
            const inp = document.getElementById('slide-input-' + i);
            if (inp) inp.value = `Inserir a imagem ${imgPath} nesse slide`;
            if (statusEl) { statusEl.textContent = 'Imagem enviada. Ajuste o pedido e clique Aplicar.'; statusEl.className = 'lightbox-slide-status ok'; }
            inp?.focus();
          } catch (err) {
            if (statusEl) { statusEl.textContent = 'Erro ao enviar: ' + (err.message || err); statusEl.className = 'lightbox-slide-status err'; }
          }
...
async function _uploadSlideImage(file) {
    ...
    reader.onload = async (e) => {
      try {
        const r = await fetch('/api/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ dataUrl: e.target.result, name: file.name }),
        });
        const data = await r.json();
