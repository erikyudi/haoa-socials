...
function writeInstagramConfig(data) {
  try {
    ...
    console.error('[MazyUI] Erro ao salvar instagram.config.json:', e.message);
    throw e;
  }
}
...
function writeAgenda(items) {
  try {
    ...
    console.error('[MazyUI] Erro ao salvar agenda.json:', e.message);
    throw e;
  }
}
...
async function handleSave(req, res) {
  try {
    ...
    const { path: rel, content, encoding } = JSON.parse(body);
...
async function handleInstagramTest(req, res) {
  ...
  try {
    const r = await fetch(`https://graph.facebook.com/v19.0/me?fields=name&access_token=${encodeURIComponent(cfg.token)}`);
...
async function publishToInstagram(item) {
  ...
  let caption = '';
  let legendaAbs;
  try { legendaAbs = safeResolve(path.join(item.carrosselPath, 'legenda.md')); } catch { legendaAbs = null; }
  if (legendaAbs && fs.existsSync(legendaAbs)) caption = fs.readFileSync(legendaAbs, 'utf8').trim();
  ...
  for (const png of pngs) {
    ...
    if (!r.ok && r.headers.get('content-type')?.includes('application/json') === false) {
      throw new Error(`Meta API (container): HTTP ${r.status}`);
    }
    const data = await r.json();
  ...
  if (!carouselRes.ok && carouselRes.headers.get('content-type')?.includes('application/json') === false) {
    throw new Error(`Meta API (carousel): HTTP ${carouselRes.status}`);
  }
  ...
  if (!publishRes.ok && publishRes.headers.get('content-type')?.includes('application/json') === false) {
    throw new Error(`Meta API (publish): HTTP ${publishRes.status}`);
  }
...
function startScheduler() {
  setInterval(async () => {
    ...
    if (changed) {
      try { writeAgenda(items); }
      catch (e) { console.error('[scheduler] Erro ao salvar agenda:', e.message); }
    }
  }, 60_000);
}
