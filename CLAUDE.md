...
No início de toda conversa, ler `projeto-ativo.json` pra identificar o
projeto em foco, depois carregar os arquivos de contexto do projeto ativo:

1. `projetos/{projeto}/_memoria/empresa.md` — quem é o cliente, o que faz, como funciona
2. `projetos/{projeto}/_memoria/preferencias.md` — tom de voz, estilo, o que evitar
3. `projetos/{projeto}/_memoria/estrategia.md` — foco atual, prioridades, prazos

Fallback (se `projetos/{projeto}/_memoria/` não existir): usar
`_memoria/empresa.md`, `_memoria/preferencias.md`, `_memoria/estrategia.md` na raiz.
...
`projetos/{projeto}/identidade/design-guide.md` como referência de estilo.
...
## Estrutura de pastas (este workspace)

```
projetos/          — um subdiretório por cliente gerenciado
  haoa/            — HAOA Corretora de Seguros
  sargentao/       — Sargentão restaurante
  vegansoulai/     — VeganSoul.ai
  {projeto}/
    _memoria/      — empresa.md, preferencias.md, estrategia.md
    identidade/    — design-guide.md, logos
    instagram.config.json
    marketing/     — conteúdo gerado por data

projeto-ativo.json — define qual projeto está em foco (ex: {"projeto":"haoa"})

scripts/           — automação de postagem Meta
  lib-meta.mjs     — wrapper Meta Graph API
  postar-instagram.js
  postar-facebook.js
  processar-agenda.js
  agendar.js

marketing/         — documentação de setup e conteúdo raiz (multi-projeto)
  automacao-meta-setup.md
  conteudo/        — peças geradas organizadas por slug+data

agenda.json        — fila de posts agendados (todos os projetos)
```

`/atualizar-sistema` nunca toca em `projetos/`, `scripts/`, `marketing/`, `agenda.json`.
